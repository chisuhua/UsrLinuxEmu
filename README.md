...
> **最后验证**: 2026-07-05 (commit `6a7f4ab`)
>
...
>
> 本 README 反映 Stage 1.4 Tier-2 完成后的状态。如发现与上述两文件冲突，以它们为准。
...
- **Stage 2**：多设备插件化（网络/存储）+ 顺势引入 IOMMU 真实硬件 invalidation（vfio）+ KFD 多文件集成评估
- **Stage 3+**：完整 KFD 多文件集成（独立子项目，~50K 行 amdgpu driver 移植）
- **v1.0 发布**：稳定 CI + 文档完善 + 性能优化
...
**当前版本**: v0.5+（Stage 1.4 Tier-2 完成）  
**最后验证**: 2026-07-05  
**对应 commit**: `6a7f4ab`（Tier-2 merge to main）  
