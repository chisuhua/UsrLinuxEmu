...
#### 概述
[device.h](include/kernel/device/device.h) 定义了所有设备的基类，提供统一的设备接口。
...
#### 概述
[file_ops.h](include/kernel/file_ops.h) 定义了文件操作的抽象接口。
...
#### 概述
[vfs.h](include/kernel/vfs.h) 提供设备节点的注册与查找机制。
...
#### 概述
[plugin_manager.h](include/kernel/plugin_manager.h) 提供插件的动态加载和管理功能。
...
#### 概述
[gpu_driver.h](drivers/gpu/gpu_driver.h) 实现 GPU 设备的驱动逻辑。
...
#### 概述
[buddy_allocator.h](drivers/gpu/buddy_allocator.h) 实现高效的内存分配算法。
...
#### 概述
[ring_buffer.h](drivers/gpu/ring_buffer.h) 实现 GPU 命令队列的环形缓冲区。
...
#### 概述
GPU 设备支持以下 IOCTL 命令，定义在 [ioctl_gpgpu.h](drivers/gpu/ioctl_gpgpu.h) 中。
...
#### 概述
[basic_gpu_simulator.h](simulator/gpu/basic_gpu_simulator.h) 提供基本的 GPU 模拟功能。
...
#### 概述
[logger.h](include/kernel/logger.h) 提供统一的日志输出接口。
...
#### 概述
[service_registry.h](include/kernel/service_registry.h) 提供全局服务注册与获取功能。
