...
**相关**: [GPU 内存管理](../05-advanced/gpu_driver_architecture.md)
...
**相关**: [GPU 驱动架构](../05-advanced/gpu_driver_architecture.md)
...
UsrLinuxEmu 使用的单元测试框架。Catch2 v2.x amalgamated 单文件版已经 vendored 在 `tests/catch_amalgamated.{hpp,cpp}`，**不需要安装任何系统包**。

```cpp
#include <catch_amalgamated.hpp>

TEST_CASE("GPU memory allocate", "[gpu][memory]") {
    GpuMemoryHandle handle;
    int ret = device->allocate_memory(1024, &handle);
    REQUIRE(ret == 0);
    REQUIRE(handle != INVALID_HANDLE);
}
```

**核心 API**:

- `TEST_CASE("name", "[tag]")` —— 测试用例声明
- `REQUIRE(expr)` —— 致命断言（失败立即终止当前 TEST_CASE）
- `CHECK(expr)` —— 非致命断言（失败继续后续断言）
- `SECTION("name")` —— 嵌套子例；Catch2 每个 SECTION 会重新执行外层 `TEST_CASE` 代码，自动获得干净状态
- `REQUIRE_NOTHROW(expr)`、`CHECK_THROWS_AS(expr, ExcType)` 用于异常路径

**文件位置**: `tests/catch_amalgamated.hpp`、`tests/catch_amalgamated.cpp`。

**测试组织**: 把用 Catch2 的测试文件加入 [`tests/CMakeLists.txt`](../../tests/CMakeLists.txt) 的 `CATCH2_TESTS` 列表；用自定义 `main()` 的加入 `STANDALONE_TESTS`。

**相关**: [测试指南](../04-building/testing_guide.md)、[build_system.md §6](../04-building/build_system.md)、[ADR-010](../00_adr/adr-010-gtest-migration.md)。
...
- [项目概述（已归档）](../archive/misc/overview-2026-08-deprecated.md) - 项目介绍
