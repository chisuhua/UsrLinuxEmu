...
**文档状态**: 顶层已由 ADR-027（v1，2026-06-16 ✅ 已接受）覆盖；任务清单已部分标注为「已弃用」
**创建日期**: 2026-03-24  
**优先级**: 高（但具体实施节奏由 ADR-027 决定）
**预计工期**: 待 ADR-027 确认
**对应总体计划**: Phase 2（gpu_driver_portability_plan.md）

---

## ADR-027 接受后的任务清单调整（2026-06-16）

按 [ADR-027 §决策 1](../00_adr/adr-027-linux-compat-strategy.md#决策-1扩展范围spec-driven按-phase-3-真实需求增量扩展) 重新分类本计划中的任务：

### ❌ 已弃用（与 ADR-027 决策 1 冲突，不再实现）

| 原计划任务 | 对应章节 | 弃用原因 |
|-----------|---------|---------|
| 任务 1.3 字符串函数 | §周 2 `string.h` | 已有 C++17 `std::string` + std::format 覆盖 |
| 任务 1.4 打印/日志函数 | §周 2 `printk.h` | 已被 `kernel::Logger` 替代 |
| 任务 2.1 字符设备 cdev | §周 4-5 | 已有 `kernel::FileOperations`（ADR-018 产物），cdev 会创建平行体系 |
| 任务 2.2 设备类 device_class | §周 5-6 | 已有 `kernel::Device`，重复抽象 |
| 任务 3.1–3.5 同步机制全套 | §第三阶段 | 已通过 `<mutex>` / `<condition_variable>` / `kernel::WaitQueue` 覆盖；spinlock 用户态无意义（无真中断） |

### ⏸️ 待触发（按 ADR-027 决策 2 排期）

| 原计划任务 | 触发条件 | 优先级 |
|-----------|---------|--------|
| 任务 4.1 PCI 兼容层 | Phase 3 网络/存储设备原型启动 | P2（每设备 1-2 周）|
| DRM 扩展至 TTM | ADR-031 TTM 迁移启动 | P1（1-2 周）|

### ✅ 仍是有效参考（未与 ADR-027 冲突）

| 原计划任务 | 章节 | 状态 |
|-----------|------|------|
| 任务 1.1 类型定义补全 | §周 1-2 | 部分已实现（types.h 60%）|
| 任务 1.2 内存管理补全 | §周 1-2 | 部分已实现（memory.h 30%）|

**任务清单更新规则**：owner 在追加 P1/P2 任务时，请在本文末尾追加「Phase 3+ 触发的新增任务」一节，而不是修改上面已弃用的章节（保留历史参考价值）。
