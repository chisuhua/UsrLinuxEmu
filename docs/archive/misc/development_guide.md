...
### 核心框架 ([include/kernel](include/kernel) 和 [src/kernel](src/kernel))

- [types.h](include/kernel/types.h): 基础类型定义
- [device.h](include/kernel/device/device.h): 设备抽象基类
- [file_ops.h](include/kernel/file_ops.h): 文件操作抽象
- [vfs.h](include/kernel/vfs.h): 虚拟文件系统接口
- [logger.h](include/kernel/logger.h): 日志系统
- [plugin_manager.h](include/kernel/plugin_manager.h): 插件管理
- [service_registry.h](include/kernel/service_registry.h): 服务注册中心

### GPU 驱动实现 ([drivers/gpu](drivers/gpu))

- [gpu_driver.h](drivers/gpu/gpu_driver.h): GPU 驱动主类
- [buddy_allocator.h](drivers/gpu/buddy_allocator.h): GPU 内存分配器
- [ring_buffer.h](drivers/gpu/ring_buffer.h): GPU 命令环形缓冲区
- [address_space.h](drivers/gpu/address_space.h): GPU 地址空间管理
- [plugin_gpu.cpp](drivers/gpu/plugin_gpu.cpp): GPU 设备插件入口

### GPU 模拟器 ([simulator/gpu](simulator/gpu))

- [basic_gpu_simulator.h](simulator/gpu/basic_gpu_simulator.h): 基础 GPU 模拟器
- [command_parser.h](simulator/gpu/command_parser.h): GPU 命令解析器

### 测试代码 ([tests](tests))

- [test_gpu_submit.cpp](tests/test_gpu_submit.cpp): GPU 命令提交测试
- [test_gpu_memory.cpp](tests/test_gpu_memory.cpp): GPU 内存管理测试
- [test_plugin.cpp](tests/test_plugin.cpp): 插件系统测试

## 添加新设备

### 1. 定义设备类

继承 [device.h](include/kernel/device/device.h) 中的 Device 基类:
...
在 [ioctl.h](include/kernel/ioctl.h) 或设备特定的头文件中定义:
