...
- 2026-07-11 v1.1: + kernel thread/workqueue 基础设施 (ADR-060)
**关联 ADR**: ADR-001, ADR-006, ADR-008, ADR-018, ADR-023, ADR-035, ADR-060
...
|------|------|---------|------|--------------|
| ① | **Linux 内核环境模拟** | `src/kernel/`, `include/kernel/`, `include/linux_compat/` | 在用户态提供 Linux 内核 API：VFS、ModuleLoader、Device 类、`linux_compat/` 类型/宏/DRM 子集、kernel thread/workqueue 基础设施 [ADR-060] | 通过 `dlopen("plugins/*.so")` 加载驱动的"宿主"；为驱动提供系统调用兼容层 |
