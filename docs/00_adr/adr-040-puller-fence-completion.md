...
- **2026-07-29 (Stage 4.5 ADR-040 迁移)**：`sim_fence_id_signal(pending_fence_id_)` 路径迁移到 timeline semaphore。`g_fence_sem_mgr` 全局指针桥接到 `SemaphoreManager::signal()`。向后兼容：sim_fence_table_ 仍同时更新，`sim_fence_id_check()` 不受影响。未来清理：迁移完成后可移除 sim_fence_table_ 的直接写路径。关联 Change: `openspec/changes/stage4-5-cp-phase6-preemption-timeline-sem/`。