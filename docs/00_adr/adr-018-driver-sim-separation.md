...
**关联 ADR**: ADR-015 (GPU IOCTL Unification), ADR-019 (DRM/GEM/TTM 对齐), ADR-023 (仿真层接口契约 HAL), ADR-036 (3-way Architectural Separation)
...
> 注：ADR-018 决策 2 的 drv→hal→sim 依赖方向规则是 ADR-036「3 区分架构原则」中 HAL 作为 ②③ 桥接层的基础。
...
- ✅ 与 ADR-036 的 3-way 分离模型一致：kernel env sim / portable driver / hardware sim
