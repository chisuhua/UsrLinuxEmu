# 开发指南

这里是开发者的主要参考文档，包含日常开发所需的所有信息。

## 导航

- [开发指南](guide.md) - 环境搭建、代码规范和开发流程
- [代码风格](coding-style.md) - 编码规范和最佳实践
- [添加新设备](adding-devices.md) - 如何扩展新设备类型
- [调试指南](debugging.md) - 调试技巧和工具
- [架构对齐报告](architecture-alignment-report.md) - 代码与架构一致性审查

## 快速导航

| 我想... | 阅读这个 |
|---------|----------|
| 搭建开发环境 | [开发指南](guide.md) |
| 了解代码规范 | [代码风格](coding-style.md) |
| 添加新设备 | [添加新设备](adding-devices.md) |
| 调试问题 | [调试指南](debugging.md) |
| 了解架构 | [核心架构 SSOT](../02_architecture/core-architecture.md) |
| 查阅 API | [API 参考](../06-reference/api-reference.md) |

## 开发工作流

1. 阅读 [开发指南](guide.md) 了解基本流程
2. 遵循 [代码风格](coding-style.md) 编写代码
3. 参考 [添加新设备](adding-devices.md) 扩展功能
4. 使用 [调试指南](debugging.md) 解决问题

---

**最后更新**: 2026-03-23
