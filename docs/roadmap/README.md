...
| **阶段 1** | ✅ 已达成 (2026-07-16) | Linux 内核环境模拟（DRM + UVM + IOMMU + ATS + PCIe BAR/中断）；C-12 KFD 多文件集成 81% 完成 + L1↔L2 bridge skeleton | [stage-1-kernel-emu.md](stage-1-kernel-emu.md) |
| **阶段 2** | ✅ 已达成 (2026-07-05) | 多设备插件化（网络 + 存储）| [stage-2-multi-device.md](stage-2-multi-device.md) |
| **阶段 3** | 🔄 进行中 | v1.0 稳定（CUDA E2E ✅、sanitizer infra、L1↔L2 bridge、性能优化、文档完善）| [stage-3-v1.0.md](stage-3-v1.0.md) |
| **终态蓝图** | 📋 愿景 | 3 区分成熟形态，可移植驱动可在真实 Linux 内核中编译运行；Stage 4: 真实 BAR + ioremap 模拟 | [blueprint.md](blueprint.md) |
...
| `three-sanitizer-infra` | 🟡 P3 | 32 tasks, 3-5d | 0/32 (待启动) | ASan/UBSan CMake infra + 修复暴露的真 bug |
| `kfd-l1-l2-bridge-e2e` | 🟡 P3 | 41 tasks, 1-2w | 0/41 (待启动) | 跨仓 KFD bridge E2E 验证 (ADR-035 §Rule 5.1) |
...
2. three-sanitizer-infra (3-5d, 单仓低风险)
3. kfd-l1-l2-bridge-e2e (1-2w, 跨仓 PR workflow)
4. Stage 3.2 性能优化 (benchmark 基线 → hotpath)
5. Stage 3.3 错误处理审计
6. Stage 3.4 文档完善 + v1.0 发布清单
```
