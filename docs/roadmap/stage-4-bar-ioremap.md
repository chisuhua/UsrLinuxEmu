...
### 4.7.1 — Phase 2 HAL Foundation ✅ 已归档
...
- [x] **`hal_user_init()` lambdas**: 委托给 `sim_graph_*` / `sim_mem_pool_*` / `sim_stream_capture_*` / `GpuQueueEmu` / `HardwarePullerEmu`（class 实例通过 `reinterpret_cast` opaque handle 还原）
- [x] **`hal_mock_init()` 默认值**: 单调计数器 + no-op 操作（确定性，可测试重现）
- [x] **0 regression**: full ctest 130/130 PASS

**Stub 范围**（由后续 removal change 真实化）:
- `mem_pool_free` — sim/ 中无对应函数（移除语义 = 释放整 pool = `mem_pool_destroy`）
- `mem_pool_alloc_async` / `mem_pool_free_async` — sim/ 占位 stub
...
- 暴露基础阶段任何 stub 缺口 → 立即反馈到 Phase 2 foundation（如果有补救 change 的需要）
- 模板建立后, #2-#5 可流水线化

### 4.7.3 — 范围外（kfd_events.c sim_event.h 违规）

**不在 Phase 2 范围** — `plugins/gpu_driver/drv/kfd/kfd_events.c` 第 1 行 `#include "../sim/sim_event.h"` 是 KFD scope 的 C-12 子项目遗留（"B.4.3: sim_signal_event() day-1 stub"）。
- 完成 #1-#5 后 L2 总数: 8 → 1（仅 sim_event.h 残留）
- 修复路径: 新增 HAL fn-ptr `sim_event_signal` 等,需要独立 proposal（不在 Stage 4.7 范围）
...
- **4.7 → 4.1-4.6**：无运行时依赖（HAL 边界修复独立于 CP 工作）。✅ 4.7.1 foundation 已 ship; 4.7.2 removals 待启动。
- **4.7.2 → 4.7.2 内部顺序**: `removal-graph` → `removal-mem-pool` → `removal-stream-capture` → `removal-gpu-queue-emu` → `removal-hardware-puller-emu`。建议第一刀先做 graph（暴露 foundation stub 缺口）再批量流水线。
