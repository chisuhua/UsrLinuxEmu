...
**ADR-064 Decision 3 触发条件**（任一满足即启动）：

1. ② 驱动代码新增 `ioremap(bar_phys_addr, size)` 调用
2. `readl`/`writel` 首次出现在驱动逻辑中（非 sim）
3. `dma_alloc_coherent` 在真实内核 API 路径被调用
4. mmu_notifier 路径需要真实 device PFN mapping（非简化堆 offset）
5. L1↔L2 bridge 跨仓测试失败于 "heap offset ≠ real PA" 语义
...
- 独立 VRAM backing store：`mmap(MAP_SHARED, fd)` 支持的持久化内存文件
- 多进程 BAR 映射隔离
...
- 创建独立 VRAM 文件 `$USRLINUXEMU_VRAM`（默认 `~/.usrlinuxemu/vram.bin`）
...
- [ ] `ioremap(BAR0_PHYS, BAR0_SIZE)` 返回有效虚拟地址
- [ ] `readl(bar0 + offset)` 读回 `writel(bar0 + offset, val)` 写入的值
- [ ] `dma_alloc_coherent()` 返回 cache 一致的物理连续内存
- [ ] 多进程打开同一 BAR 时映射隔离
- [ ] 测试：`tests/test_bar_ioremap_standalone` + `tests/test_dma_coherent_standalone`
...
- [ ] CP 可移植性边界文档（ADR-043：`drv/` 与 `sim/` 之间的 CP API 白名单）
...
- [ ] 优先级调度（ADR-045：per-queue priority + starvation prevention）
- [ ] Hardware Semaphore & Barrier Model（ADR-047：跨 stream 同步）
- [ ] Indirect Buffer 命令链（ADR-050：IB chain walking）
...
```
4.1 BAR + ioremap ──> 4.2 CP Phase 4 (Graph launch) ──> 4.3 CP Phase 5 (Method + HyperQueue)
                                                               │
                                                               ├──> 4.4 CP Phase 5.5 (Priority + Semaphore)
                                                               │
                                                               └──> 4.5 CP Phase 6 (Preemption + Cross-engine) ──> 4.6 CP Phase 7 (Green Context)
```

- **4.1 → 4.2**：真实 BAR 映射后，Graph launch 才能使用 `ioremap` 习语访问 VRAM 中的 graph node
- **4.2 → 4.3**：CP 可移植性边界建立后，方法编解码和调度才有正确的抽象层
- **4.3 → 4.4 + 4.5**：基础调度（Phase 5）就绪后，高级特性（优先级/抢占）才有底座
- **4.5 → 4.6**：跨引擎同步可用后，Green Context 多 context 并发调度才能正确 fence
...
| 风险 | 概率 | 影响 | 缓解 |
|------|------|------|------|
| BAR 模拟性能开销大 | 中 | 中 | mmap(MAP_SHARED) 而非每次 syscall；性能基准纳入 CI |
| GPU CP Phase 4-7 工作量大 | 高 | 高 | 按 Phase 递进交付，每 Phase 自成里程碑 |
| ioremap 习语与真实内核 API 不一致 | 中 | 高 | 参考 Linux 6.12 LTS `arch/x86/mm/ioremap.c` 实现，API 签名严格对齐 |
| 多进程 BAR 映射隔离复杂 | 低 | 中 | v1.0 单进程即可；多进程隔离按需迭代 |
