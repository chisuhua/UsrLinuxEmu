...
GPU 驱动模块在 [drivers/gpu/CMakeLists.txt](drivers/gpu/CMakeLists.txt) 中定义：
...
插件系统支持动态加载，插件构建在 [plugins/](plugins) 目录下：
...
测试构建在 [tests/CMakeLists.txt](tests/CMakeLists.txt) 中定义：
...
命令行工具构建在 [tools/cli/CMakeLists.txt](tools/cli/CMakeLists.txt) 中：
...
1. 在 [drivers/](drivers) 目录创建新驱动目录
2. 实现驱动代码和头文件
3. 创建对应的 CMakeLists.txt
4. 修改主 CMakeLists.txt 或 [drivers/CMakeLists.txt](drivers/CMakeLists.txt) 包含新驱动
