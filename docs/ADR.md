...
### ADR-015: GPU IOCTL 接口统一（已接受）
- 确立 System C (GPU_IOCTL_*) 为 canonical 接口
- 废弃 System A (CUDA_IOCTL_*) 和 System B (GPGPU_*)
- 定义 6 个 P0 命令：GET_DEVICE_INFO, ALLOC_BO, FREE_BO, MAP_BO, PUSHBUFFER_SUBMIT_BATCH, WAIT_FENCE
- See: ADR-015-gpu-ioctl-unification.md

### ADR-016: GPU Memory Domain 模型（已接受）
- 定义 VRAM/GTT/CPU 三层 domain 模型
- ALLOC_BO 支持 domain 参数
- See: ADR-016-gpu-memory-domain.md

### ADR-017: GPFIFO/Queue 抽象（已接受）
- 定义 GpuVaSpace 和 GpuQueue 抽象
- Phase 1 隐式单队列，Phase 2 显式队列
- See: ADR-017-gpfifo-queue-abstraction.md
...
- 2026-04-28: ADR-015 (GPU IOCTL Unification), ADR-016 (GPU Memory Domain), ADR-017 (GPFIFO/Queue) 已接受
- 2026-04-28: Phase 0 修复完成 — gpu_ioctl.h 增加 domain 字段、WAIT_FENCE、VA Space/Queue ioctl 定义
