...
section_arch() {
    ...
    subsection "1.2 src/kernel cpp file count (expected 42 post-Stage 2 multi-device)"
    local count
    ...
    if [ "${count}" -eq 42 ]; then
        ...
        check_warn "src/kernel has ${count} cpp files (baseline 42; update after adding new kernel modules)"
    fi
