...
BuddyAllocator::BuddyAllocator(uint64_t base, size_t total_size)
  ...
  std::cout << "[BuddyAllocator] Initialized with base=0x" << std::hex << base_ << ", size=0x"
            << total_size_ << std::dec << std::endl;
}
...
int BuddyAllocator::allocate(size_t size, uint64_t* addr_out) {
  ...
  *addr_out = blocks_[idx].addr;
  ...
  std::cout << "[BuddyAllocator] Allocated: 0x" << std::hex << *addr_out << " (" << std::dec
            << aligned_size << " bytes)" << std::endl;
