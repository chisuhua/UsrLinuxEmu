...
int main() {
  ...
  // 分配显存
  struct gpu_alloc_bo_args alloc_args = {.size = 128 * 1024,  // 128KB
  ...
  if (ret == 0) {
    std::cout << "[TestGPU] Allocated BO: handle=" << alloc_args.handle << " va=0x" << std::hex
              << alloc_args.gpu_va << std::dec << std::endl;
  } else {
  ...
  // 等待 fence
  struct gpu_wait_fence_args fence_args = {.fence_id = 1, .timeout_ms = 1000, .status = 0};
