...
SampleAnalysis analyse(const IConfig& cfg, FDuration* first, FDuration* last) {
  if (!cfg.benchmarkNoAnalysis()) {
    ...
    auto outliers = Catch::Benchmark::Detail::classify_outliers(samples.data(),
                                                                samples.data() + samples.size());
...
static sample resample(URng& rng, unsigned int resamples, double const* first, double const* last,
                       Estimator& estimator) {
...
static sample jackknife(double (*estimator)(double const*, double const*), double* first,
                        double* last) {
...
bootstrap_analysis analyse_samples(double confidence_level, unsigned int n_resamples, double* first,
                                   double* last) {
...
Config::Config(ConfigData const& data) : m_data(data) {
  ...
  if (m_data.reporterSpecifications.empty()) {
    ...
    CATCH_ENFORCE(parsed,
                  "Cannot parse the provided default reporter spec: '" << default_spec << '\'');
...
IEventListenerPtr prepareReporters(Config const* config) {
  if (Catch::getRegistryHub().getReporterRegistry().getListeners().empty() &&
    ...
    return createReporter(spec.name, ReporterConfig(config, makeStream(spec.outputFilename),
  ...
  for (auto const& reporterSpec : config->getProcessedReporterSpecs()) {
    multi->addReporter(createReporter(
        reporterSpec.name, ReporterConfig(config, makeStream(reporterSpec.outputFilename),
  ...
  if (tokens && tokens->type == Detail::TokenType::Option) {
    ...
    if (isMatch(token.token)) {
      if (m_ref->isFlag()) {
        ...
        ++tokens;
        if (!tokens)
          return Detail::InternalParseResult::runtimeError("Expected argument following " +
                                                           token.token);
...
Clara::Parser makeCommandLineParser(ConfigData& config) {
  ...
  auto cli =
      ...
      Opt(setDefaultColourMode,
          "ansi|win32|none|default")["--colour-mode"]("what color mode should be used as default") |
      ...
      Opt(setShardCount,
          "shard count")["--shard-count"]("split the tests to execute into this many groups") |
...
class StreamRedirect : public OutputRedirect {
 ...
 public:
  ...
  std::string getStdout() override {
    return m_redirectedOut.str();
  }
  std::string getStderr() override {
    return m_redirectedErr.str();
  }
  void clearBuffers() override {
    m_redirectedOut.str("");
    m_redirectedErr.str("");
...
class FileRedirect : public OutputRedirect {
 ...
 public:
  ...
  std::string getStdout() override {
    return m_outFile.getContents();
  }
  std::string getStderr() override {
    return m_errFile.getContents();
  }
  void clearBuffers() override {
    m_outFile.clear();
    m_errFile.clear();
...
std::vector<Detail::unique_ptr<EventListenerFactory>> const& ReporterRegistry::getListeners()
    const {
...
kvPair splitKVPair(StringRef kvString) {
  auto splitPos =
      static_cast<size_t>(std::find(kvString.begin(), kvString.end(), '=') - kvString.begin());
...
std::vector<std::string> splitReporterSpec(StringRef reporterSpec) {
  ...
  while (separatorPos < reporterSpec.size()) {
    ...
    parts.push_back(
        static_cast<std::string>(reporterSpec.substr(separatorPos, nextSeparator - separatorPos)));
...
Optional<ReporterSpec> parseReporterSpec(StringRef reporterSpec) {
  ...
  return ReporterSpec{CATCH_MOVE(parts[0]), CATCH_MOVE(outputFileName), CATCH_MOVE(colourMode),
                      CATCH_MOVE(kvPairs)};
}
...
struct GeneratorTracker final : TestCaseTracking::TrackerBase, IGeneratorTracker {
  ...
  GeneratorTracker(TestCaseTracking::NameAndLocation&& nameAndLocation, TrackerContext& ctx,
                   ITracker* parent)
...
void RunContext::handleFatalErrorCondition(StringRef message) {
  ...
  // Since this is a fatal error, we have not had and won't have the opportunity to destruct them
  // properly
...
static void enforceNoDuplicateTestCases(std::vector<TestCaseHandle> const& tests) {
  ...
  for (auto const& test : tests) {
    ...
    CATCH_ENFORCE(prev.second, "error: test case \""
                                   << infoPtr->name << "\", with tags \"" << infoPtr->tagsAsString()
                                   << "\" already defined.\n"
...
SectionTracker& SectionTracker::acquire(TrackerContext& ctx,
  ...
  if (ITracker* childTracker = currentTracker.findChild(nameAndLocation)) {
    ...
    auto newTracker = Catch::Detail::make_unique<SectionTracker>(
        NameAndLocation{static_cast<std::string>(nameAndLocation.name), nameAndLocation.location},
        ctx, &currentTracker);
...
std::string Column::const_iterator::addIndentAndSuffix(
    AnsiSkippingString::const_iterator start, AnsiSkippingString::const_iterator end) const {
  std::string ret;
...
std::string WithinRelMatcher::describe() const {
  Catch::ReusableStringStream sstr;
  sstr << "and " << ::Catch::Detail::stringify(m_target) << " are within " << m_epsilon * 100.
       << "% of each other";
  return sstr.str();
}
...
class TablePrinter {
 ...
 public:
  ...
  friend TablePrinter& operator<<(TablePrinter& tp, ColumnBreak) {
    ...
    auto padding = (strSize + 1 < colInfo.width) ? std::string(colInfo.width - (strSize + 1), ' ')
                                                 : std::string();
...
void printTestRunTotals(std::ostream& stream, ColourImpl& streamColour, Totals const& totals) {
  ...
  columns.push_back(
      SummaryColumn("", Colour::None).addRow(totals.testCases.total()).addRow(totalAssertionCount));
...
void MultiReporter::assertionEnded(AssertionStats const& assertionStats) {
  const bool reportByDefault = assertionStats.assertionResult.getResultType() != ResultWas::Ok ||
...
void SonarQubeReporter::writeSection(std::string const& rootName, SectionNode const& sectionNode,
                                     bool okToFail) {
  std::string name = trim(sectionNode.stats.sectionInfo.name);
  if (!rootName.empty())
    name = rootName + '/' + name;
