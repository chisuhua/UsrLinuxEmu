# stage4-l2-foundation-removal-graph Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use skill_use("execute") to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Remove L2 violation `#include "sim/graph.h"` from drv/ files and replace direct sim calls with HAL fn-ptr wrappers. Per ADR-072 §Decision 4 revised + ADR-023 Decision 4 (opaque handle abstraction).
...
| File | Responsibility |
|---|---|
| `plugins/gpu_driver/drv/gpgpu_device.cpp` | Remove `sim/graph.h` include; replace sim calls with hal_* wrappers |
| `plugins/gpu_driver/drv/gpu_drm_driver.cpp` | (if applicable) Remove `sim/graph.h` include; replace sim calls |
| `plugins/gpu_driver/hal/hal_user.cpp` | (if applicable) Realize hal_graph_* lambdas |
...
| File | Responsibility |
|---|---|
| `tests/test_*_standalone.cpp` | Run existing graph tests; add regression constraint if applicable |
...
grep -n 'sim_graph' plugins/gpu_driver/drv/gpgpu_device.cpp plugins/gpu_driver/drv/gpu_drm_driver.cpp
grep -n 'hal_graph' plugins/gpu_driver/hal/hal_user.cpp plugins/gpu_driver/hal/hal_mock.cpp
```
...
Source: `openspec/changes/stage4-l2-foundation-removal-graph/tasks.md`
