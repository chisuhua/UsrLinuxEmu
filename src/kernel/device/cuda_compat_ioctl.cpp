...
int cuda_compat_ioctl(unsigned long request, void* argp) {
  ...
  switch (request) {
    ...
    case CUDA_IOCTL_MEM_ALLOC: {
      ...
      std::cout << "[CUDACompat] MEM_ALLOC: size=" << req->size << " → ptr=0x" << std::hex
                << device_ptr << std::dec << " fence=" << req->fence_id << "\n";
      break;
    }
    ...
    case CUDA_IOCTL_MEM_FREE: {
      ...
      if (result != taskrunner::CudaResult::SUCCESS) {
        std::cerr << "[CUDACompat] mem_free failed (ptr=0x" << std::hex << req->device_ptr
                  << std::dec << ")\n";
        return -cuda_result_to_errno(result);
      }
      ...
      std::cout << "[CUDACompat] MEM_FREE: ptr=0x" << std::hex << req->device_ptr << std::dec
                << " fence=" << req->fence_id << "\n";
      break;
    }
    ...
    case CUDA_IOCTL_MEMCPY_H2D: {
      ...
      std::cout << "[CUDACompat] MEMCPY_H2D: ptr=0x" << std::hex << req->device_ptr << std::dec
                << " offset=" << req->offset << " size=" << req->size << " fence=" << req->fence_id
                << "\n";
      break;
    }
    ...
    case CUDA_IOCTL_MEMCPY_D2H: {
      ...
      std::cout << "[CUDACompat] MEMCPY_D2H: ptr=0x" << std::hex << req->device_ptr << std::dec
                << " offset=" << req->offset << " size=" << req->size << " fence=" << req->fence_id
                << "\n";
      break;
    }
    ...
    case CUDA_IOCTL_LAUNCH_KERNEL: {
      ...
      std::cout << "  grid=" << req->grid_dim_x << "x" << req->grid_dim_y << "x" << req->grid_dim_z
                << "\n";
      std::cout << "  block=" << req->block_dim_x << "x" << req->block_dim_y << "x"
                << req->block_dim_z << "\n";
