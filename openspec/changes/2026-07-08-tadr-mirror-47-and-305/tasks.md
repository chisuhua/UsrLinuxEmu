...
- [ ] **2.2** 在 `tadr-304` 行后新增 `tadr-305` 行（line 292 后）
  ```markdown
...
- [ ] **2.3** 验证所有 `../external/TaskRunner/docs/shared/adr/tadr-*.md` 链接可解析
  ```bash
  # 列出所有 tadr 链接并验证
...
- [ ] **3.1** 修改 `docs/00_adr/adr-039-mem-pool-export-ioctl.md:8`
  ```diff
...
- [ ] **3.2** 修改 `docs/00_adr/adr-039-mem-pool-export-ioctl.md:83`
  ```diff
...
- [ ] **3.3** 全仓搜索遗留 `tadr-302` mempool 引用（应只剩 sync-primitives）
  ```bash
...
- [ ] 所有 phase 任务完成
- [ ] `git log` 显示本 change 关联的 2 个 commit 已落地
- [ ] `openspec list` 不再显示本 change（已 archive）
- [ ] `openspec/changes/2026-07-08-tadr-mirror-47-and-305/` 移动到 `openspec/changes/archive/2026-07-08-2026-07-08-tadr-mirror-47-and-305/`
