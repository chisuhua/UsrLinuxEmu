...
- [ ] 1.1 Run `ctest --output-on-failure` from `build/` and pin the baseline
  count (expected: 145/145 PASS, but verify at the moment of execution).
- [ ] 1.2 Run `tools/check-portability.sh` from project root and confirm it
  currently PASSES (L1 baseline).
- [ ] 1.3 Run `grep -rn '#include.*cudart\|#include.*cpptlm'
  plugins/gpu_driver/drv/` and confirm empty output (boundary baseline).
- [ ] 1.4 Confirm HEAD of `external/PTX-EMU/` is at or beyond tag `v0.1.0` (the
...
- [ ] 2.1.1 **Write failing test** — add a `static_assert` or compile-time
...
- [ ] 2.1.2 **Verify fail**: build fails with "negative width" or
  `static_assert` failure citing 65 vs 68. Capture exact error in commit.
- [ ] 2.1.3 **Implement**: in `plugins/gpu_driver/hal/gpu_hal.h`, append 3
...
- [ ] 2.1.4 **Verify pass**: rebuild — `static_assert` now succeeds.
- [ ] 2.1.5 **Commit**: `feat(hal): append 3 kernel-module fn-ptrs to gpu_hal_ops`.

### 2.2 Update `hal_mock.cpp` with mock implementations (spec: "HAL mock layer with injectable error semantics")

- [ ] 2.2.1 **Write failing test** — extend
...
- [ ] 2.2.2 **Verify fail**: test fails with `hal->kernel_module_load == nullptr`
  for at least one mock.
- [ ] 2.2.3 **Implement**: append 3 mock fn-ptr bodies to `hal_mock.cpp`. Each
...
- [ ] 2.2.4 **Verify pass**: rebuild, rerun — SECTION passes.
- [ ] 2.2.5 **Commit**: `feat(hal-mock): append 3 mock implementations + error injection map`.

## 3. ioctl number assignment (TDD)

### 3.1 Add 3 ioctl codes 0x27-0x29 to `shared/gpu_ioctl.h` (spec: "New ioctl codes 0x27-0x29")

- [ ] 3.1.1 **Write failing test** — extend
...
- [ ] 3.1.2 **Verify fail**: test output reports "no handler for
  `GPU_IOCTL_LOAD_KERNEL_MODULE` (0x27)".
- [ ] 3.1.3 **Implement**: append to `plugins/gpu_driver/shared/gpu_ioctl.h`:
  ```c
  ...
  ```
- [ ] 3.1.4 **Verify pass**: ioctl-table-coverage test still fails (codes exist
  but no handler entries yet — expected, defer to §4).
- [ ] 3.1.5 **Commit**: `feat(ioctl): add 3 kernel-module ioctls + MAX_KERNEL_IMAGE_SIZE`.

## 4. drv/ ioctl dispatch (TDD)

### 4.1 Bump `kNumIoctls` 38→41 and add 3 IoctlEntry rows (spec: "Append-only ioctl dispatch extension")

- [ ] 4.1.1 **Write failing test** — extend
  ...
  0x27-0x29.
- [ ] 4.1.2 **Verify fail**: dispatch returns `-EINVAL` for the 3 new codes.
- [ ] 4.1.3 **Implement**:
...
- [ ] 4.1.4 **Verify pass**: ioctl-table-coverage test passes for all 41 codes.
- [ ] 4.1.5 **Commit**: `feat(drv): add 3 kernel-module ioctl handlers + bump kNumIoctls 38→41`.
...
- [ ] 5.1.1 **Write failing test** — add SECTION 1 to
...
- [ ] 5.1.2 **Verify fail**: test logs "expected `-ENOSYS` got `<other>`".
- [ ] 5.1.3 **Implement**: in `plugins/gpu_driver/hal/hal_user.cpp`:
...
- [ ] 5.1.4 **Verify pass**: SECTION 1 (dlsym failure → -ENOSYS) passes.
- [ ] 5.1.5 **Commit**: `feat(hal-user): real dlsym of libptxemu_device.so + 3 fn-ptr bodies + canonical error map`.

### 5.2 Add additional SECTIONs (spec: test scenarios for SUCCESS + ROLLBACK + BOUNDS)

- [ ] 5.2.1 SECTION 2: version mismatch → `-EPROTO` (mock a lib whose
  `module_version` returns 0; assert rc == `-EPROTO`). Required when a mock
  PTX-EMU is available; otherwise pin to `-ENOSYS` and document.
- [ ] 5.2.2 SECTION 3: concurrent `ensure_loaded` race — N=8 threads call
  `hal_kernel_module_load` simultaneously; assert no data race (run under
  TSan to verify).
- [ ] 5.2.3 SECTION 4: in-flight unload → `-EBUSY`. Use mock backend with a
  handle flagged "in-flight"; assert `image_unload` returns the mapped
  errno.
- [ ] 5.2.4 SECTION 5: image_size boundary. Pass `image_size = 0`,
...
- [ ] 5.2.5 SECTION 6: kernel-name rollback no-leak. Mock
...
- [ ] 5.2.6 Verify all 6 SECTIONs PASS, commit:
...
- [ ] 6.1.1 This step is not test-driven; it's a build-system verification.
...
- [ ] 6.1.2 Restore: `plugins/gpu_driver/CMakeLists.txt:33` → add
  `${CMAKE_DL_LIBS}` to the target's `target_link_libraries`.
- [ ] 6.1.3 Rebuild → link succeeds.
- [ ] 6.1.4 Commit: `build(gpu_driver): link against CMAKE_DL_LIBS for glibc < 2.34 compat`.

## 7. Integration & regression

- [ ] 7.1 Run `make -j4` from `build/` and confirm zero compile warnings on
  changed files.
- [ ] 7.2 Run `ctest --output-on-failure` from `build/`. Baseline 145/145 +
  3 new binaries (each with multiple SECTIONs) — expect all green.
- [ ] 7.3 Run `tools/check-portability.sh` and confirm L1 PASS +
  `grep -rn '#include.*cudart\|cpptlm' plugins/gpu_driver/drv/` is empty.
- [ ] 7.4 Run `lsp_diagnostics` on `gpgpu_device.{h,cpp}`, `hal_user.cpp`,
  `hal_mock.cpp`, `gpu_hal.h`, `gpu_ioctl.h`, and all 3 new test files —
  expect no errors.
- [ ] 7.5 Optional: build with `SANITIZER=tsan` for the new test binary only
  and confirm TSan reports zero races in SECTION 3.

## 8. Documentation & governance sync

- [ ] 8.1 Amend `docs/00_adr/adr-076-gpgpu-kernel-module-ioctl.md`:
...
- [ ] 8.2 Update `docs/00_adr/README.md` index: PROPOSED count 5→4 (or
  whatever the current count is at integration time).
- [ ] 8.3 No new `static_assert` against existing 65 fn-ptrs (governance
  check; L1 portability script validates).
- [ ] 8.4 Cross-link the new spec
...
- [ ] 9.1 Single atomic commit for the UsrLinuxEmu side, scope-prefixed:
  `feat(gpu-hal): append kernel_module {load,execute,unload} via PTX-EMU
  libptxemu_device.so dlsym (#66-#68)`.
- [ ] 9.2 Do NOT touch `external/PTX-EMU/` (PTX-EMU is owner); no commit in
  that submodule.
- [ ] 9.3 Do NOT touch `external/TaskRunner/` (tadr-307 is independent
  change). Document the SOFT gate status in the ADR-076 amend.

## 10. plan-done verification

- [ ] 10.1 All 3 artifacts (`proposal.md`, `design.md`, `tasks.md`) committed
...
- [ ] 10.3 Run `openspec validate add-ptxemu-kernel-module-hal-extension`
  and confirm PASS.
- [ ] 10.4 Run guide-plan → plan-done gate (≥1 change + all artifacts
