...
> **状态**: ✅ COMPLETED（2026-07-18，归档 2026-07-22）
...
- [x] A.1.1 复用 `tests/test_kfd_dispatch_standalone.cpp` 的 mock handler 模式
- [x] A.1.2 在 `tests/test_kfd_l1_l2_bridge_standalone.cpp` 实现 VFS+Plugin fixture 作为 GpuDriverClient stub
...
- [x] A.1.3 VFS + PluginLoader setup fixture（参考 `test_module_load_and_vfs_standalone`）
- [x] A.1.4 设备 fd 获取 fixture
...
- [x] A.0.1 在 `gpgpu_device.cpp` 的 `getIoctlTablePtr()` 中为 GPU_IOCTL_GET_PROCESS_APERTURE 添加 IoctlEntry（handler 委托到 `kfd_sim_handle_get_process_aperture`）
- [x] A.0.2 添加 GPU_IOCTL_UPDATE_QUEUE IoctlEntry（handler 委托到 `kfd_sim_handle_update_queue`）
- [x] A.0.3 添加 GPU_IOCTL_MAP_MEMORY IoctlEntry（handler 委托到 `kfd_sim_handle_map_memory`）
- [x] A.0.4 添加 GPU_IOCTL_UNMAP_MEMORY IoctlEntry（handler 委托到 `kfd_sim_handle_unmap_memory`）
- [x] A.0.5 递增 `kNumIoctls` 常量 + 验证 `getIoctlTablePtr()` 数组长度匹配
- [x] A.0.6 `cmake --build build --target gpu_driver` 0 errors + lsp_diagnostics clean

### A.2 真实 E2E 测试（替换 skeleton）

- [x] A.2.1 **替换 skeleton Test 1**：GpuDriverClient stub → `GPU_IOCTL_MAP_MEMORY` → kfd_sim_bridge 端到端
...
- [x] A.2.2 **替换 skeleton Test 2**：5 个 KFD ioctls 端到端（GpgpuDevice IoctlEntry 表已含全部 5 条，见 A.0）
...
- [x] A.2.3 **替换 skeleton Test 3**：并发 kfd_sim_bridge 多线程访问
...
- [x] A.3.1 `cmake --build build --target test_kfd_l1_l2_bridge_standalone` 0 errors
- [x] A.3.2 `./build/bin/test_kfd_l1_l2_bridge_standalone` all PASS
  - **每个 TEST_CASE 必须包含具体断言值**（如 `REQUIRE(ret == 0)`、`CHECK(pfn == expected_pfn)`），不得使用 `CHECK(pfn == 0 || pfn != 0)` 这类永远为真的骨架断言
- [x] A.3.3 `cd build && ctest -j4` 104/104 PASS（含新增 assertions）
- [x] A.3.4 docs-audit 43/43 PASS
...
- [x] B.1.1 在 `external/TaskRunner/openspec/changes/l1-l2-bridge-e2e-test-skeleton/` 创建目录（与 `archive/2026-07-12-*` 互补，不冲突）
- [x] B.1.2 写 proposal.md（描述 TaskRunner 端 E2E 验证 + 引用 UsrLinuxEmu 本 change）
- [x] B.1.3 写 tasks.md（TaskRunner 端任务分解）
- [x] B.1.4 验证 TaskRunner openspec list 显示新 change

### B.2 TaskRunner 端 E2E 实现

- [x] B.2.1 GpuDriverClient 端添加 E2E test（调用 UsrLinuxEmu ioctl）
- [x] B.2.2 测试 5 个 KFD ioctls 真实端到端调用
- [x] B.2.3 验证 TaskRunner ctest 仍 10/10 PASS

### B.3 TaskRunner PR + Merge

- [x] B.3.1 `git checkout -b kfd-l1-l2-bridge-e2e`
- [x] B.3.2 实施 + 验证 + commit
- [x] B.3.3 `gh pr create`（TaskRunner 仓 PR）
- [x] B.3.4 review + merge 到 main
- [x] B.3.5 `openspec archive l1-l2-bridge-e2e-test-skeleton`
...
- [x] C.1.1 在 UsrLinuxEmu 本仓创建 submodule bump change 或 commit
- [x] C.1.2 `git submodule update --remote external/TaskRunner` → 指向 TaskRunner merge commit
- [x] C.1.3 验证 UsrLinuxEmu 104/104 + TaskRunner 10/10 ctest 双绿
- [x] C.1.4 commit + push（UsrLinuxEmu 仓）

### C.2 TaskRunner → UsrLinuxEmu bump（如需要）

- [x] C.2.1 TaskRunner 仓 submodule（指向 UsrLinuxEmu）bump 到 merge commit
- [x] C.2.2 验证双仓 ctest 双绿
- [x] C.2.3 commit + push（TaskRunner 仓）

## Phase D: 文档 + 归档（0.5-1 天）

### D.1 文档更新

- [x] D.1.1 `docs/07-integration/taskrunner-index.md` 更新（双向 sync 状态）
- [x] D.1.2 `docs/05-advanced/kfd-portability-boundary.md` v1.4 更新（E.2.4 ✅ completed）
- [x] D.1.3 `docs/superpowers/plans/sync-plan.md` 更新（如适用）
- [x] D.1.4 docs-audit 43/43 PASS

### D.2 归档（ADR-035 §Rule 5.1 Step 4）

- [x] D.2.1 UsrLinuxEmu: `openspec archive 2026-07-16-kfd-l1-l2-bridge-e2e`
- [x] D.2.2 INDEX.md 更新 C-12 follow-up 完成

## Phase E: 验收（0.5 天）

- [x] E.1 UsrLinuxEmu 104/104 ctest PASS（含具体断言值，无 `||` 永远为真断言）
- [x] E.2 TaskRunner 10/10 ctest PASS
- [x] E.3 docs-audit 43/43 PASS
- [x] E.4 双仓 INDEX.md 同步
- [x] E.5 ADR-035 §Rule 5.1 4-step 全部完成
