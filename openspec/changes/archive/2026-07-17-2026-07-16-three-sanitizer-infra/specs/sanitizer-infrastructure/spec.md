...
> **Capability**: `sanitizer-infrastructure`
> **Type**: New capability
> **Status**: PROPOSED
> **Change**: `2026-07-16-three-sanitizer-infra`

## Purpose

为 UsrLinuxEmu 提供可复现、可隔离、可验证的 ASan、UBSan 和 TSan 构建与测试基础设施，覆盖 kernel、driver、HAL、sim、plugin、tests、tools 和 `libgpu_core` 目标，同时避免共享插件产物导致的跨构建污染。本 capability 仅支持 **Linux / ELF** 平台（依赖 `nm`、`.so` 和 GCC/Clang 工具链）。
