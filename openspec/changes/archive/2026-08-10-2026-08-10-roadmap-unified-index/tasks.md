# Tasks: Roadmap Unified Index

## Phase 0 (Task 1-4 of plan): rdd-workflow artifacts
- [x] Task 1: Extend ci-docs-audit (failing test)
- [x] Task 2: Create self-referential improvement
- [x] Task 3: Reactivate proposal-suggestions
- [x] Task 4: Create OpenSpec change artifacts

## Phase 1-9 (Task 5-12 of plan): Implementation
- [x] Task 5: roadmap.md upgrade (4 段新增 + 152 REPLACE)
- [x] Task 6: stage-4 4-quadrant pilot
- [x] Task 7: stage-5 4-quadrant + cross-repo section
- [x] Task 8: stage-0/1/2/3 4-quadrant
- [x] Task 9: proposal-approved fix + B trace
- [x] Task 10: adr-076 C+D fixes
- [x] Task 11: Final verification (4 audits + 98 binaries)
- [x] Task 12: Archive change
