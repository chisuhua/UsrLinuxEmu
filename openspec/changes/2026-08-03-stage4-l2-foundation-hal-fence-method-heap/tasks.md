...
- [x] 1.1 Add 5 fn-ptr declarations to `struct gpu_hal_ops` (append-only, after `hal_pdl_signal_completion`):
...
- [x] 1.2 Add 5 inline wrapper functions in `gpu_hal.h`:
...
- [x] 2.1 In `hal_user_init()` (or equivalent), after existing fn-ptr assignments:
...
- [x] 2.2 Verify includes: `sim/fence_id.h` + `sim/hardware/method_codec.h` + `hal/hal_user.h` already present in hal_user.cpp

## 3. Mock implementation (hal_mock.cpp)

- [x] 3.1 In `hal_mock_init()` (or equivalent), after existing fn-ptr assignments:
...
- [x] 3.2 Verify includes: `<atomic>` (likely already present for `std::atomic`)

## 4. Verification

- [x] 4.1 `make kernel gpu_hal hal_mock hal_user gpu_sim plugin_gpu_driver -j4` — clean build
- [x] 4.2 `make test` — all existing tests still PASS (0 regression)
- [x] 4.3 Confirm new fn-ptrs are initialized in BOTH `hal_user_init()` and `hal_mock_init()` (no null fn-ptrs)
- [x] 4.4 L2 violation count: still 12 (this change doesn't remove any — that is follow-up)
- [x] 4.5 `docs-audit.sh --strict` — clean (except Doxygen-not-installed env limitation)

## 5. Documentation

- [x] 5.1 (Not in scope for this change) Update `docs/00_adr/adr-023-hal-interface.md` Decision 4 spec to list 5 new fn-ptrs
- [x] 5.2 (Not in scope) Update `docs/00_adr/adr-072-portability-validation.md` — note that 5 fn-ptrs are available, 3 removal changes pending

## 6. Archive

- [x] 6.1 `openspec archive 2026-08-03-stage4-l2-foundation-hal-fence-method-heap --yes --skip-specs`
- [x] 6.2 (Not in scope) `openspec change 2026-08-03-stage4-l2-foundation-removal-fence-id --yes` (separate change)
- [x] 6.3 (Not in scope) `openspec change 2026-08-03-stage4-l2-foundation-removal-method-codec --yes` (separate change)
- [x] 6.4 (Not in scope) `openspec change 2026-08-03-stage4-l2-foundation-removal-hal-user --yes` (separate change)
