...
- [x] 创建 `plugins/pci_driver/` 目录
- [x] 创建 `plugins/pci_driver/include/` 子目录
- [x] 创建 `plugins/pci_driver/CMakeLists.txt`（见 design.md §D4.1）
- [x] 创建 `plugins/pci_driver/plugin.cpp` 骨架（含 module mod 导出）

### 任务 1.2：迁移公共头文件

- [x] **Write test**: `tests/plugins/test_pci_driver_standalone.cpp::test_header_exists` 验证公共头文件存在
- [x] **Verify fail**: 测试失败（公共头未迁移）
- [x] **Implement**:
  ...
  - [ ] 验证内容相同（`diff`）
- [x] **Verify pass**: 测试通过
- [x] **Commit**: `feat(pci-driver): migrate public headers from include/kernel/pcie/`

### 任务 1.3：迁移私有头文件

- [x] **Write test**: `tests/plugins/test_pci_driver_standalone.cpp::test_impl_header_exists`
- [x] **Verify fail**: 测试失败
- [x] **Implement**:
  - [ ] `cp src/kernel/pcie/pcie_emu_impl.h plugins/pci_driver/include/pcie_emu_impl.h`
- [x] **Verify pass**: 测试通过
- [x] **Commit**: `feat(pci-driver): migrate private header pcie_emu_impl.h`

### 任务 1.4：迁移 4 个实现源文件

- [x] **Write test**: `tests/plugins/test_pci_driver_standalone.cpp::test_basic_pcie_emu` 验证 PcieEmu 基本功能
- [x] **Verify fail**: 测试失败（实现未迁移）
- [x] **Implement**:
...
- [x] **Verify pass**: 测试通过（编译 + 运行）
- [x] **Commit**: `feat(pci-driver): migrate 4 implementation files from src/kernel/pcie/`

### 任务 1.5：重命名命名空间

- [x] **Write test**: `tests/plugins/test_pci_driver_standalone.cpp::test_namespace_pci` 验证 `usr_linux_emu::pci` 命名空间
- [x] **Verify fail**: 测试失败（命名空间未改）
- [x] **Implement**:
  - [ ] 在 `plugins/pci_driver/` 全部文件中将 `pcie_internal` → `pci`
  - [ ] 更新 `include/linux_compat/pci/` 中的引用
- [x] **Verify pass**: 测试通过 + 现有 test_pcie_emu_standalone 仍 PASS（API 兼容）
- [x] **Commit**: `refactor(pci-driver): rename namespace pcie_internal -> pci`

### 任务 1.6：移除 src/kernel/pcie/ 旧文件

- [x] **Write test**: `grep -r "src/kernel/pcie/" --include="*.cpp" --include="*.h"` 应返回 0 匹配（除归档注释）
- [x] **Verify fail**: 测试失败（仍有引用）
- [x] **Implement**:
  - [ ] `rm -rf src/kernel/pcie/`
  - [ ] 修改 `src/kernel/CMakeLists.txt`：移除 pcie 引用
- [x] **Verify pass**: 全仓编译通过 + 全部现有测试 PASS
- [x] **Commit**: `refactor(kernel): remove src/kernel/pcie/ after migration`
...
- [x] 创建 `plugins/iommu_driver/` 目录
- [x] 创建 `plugins/iommu_driver/include/` 子目录
- [x] 创建 `plugins/iommu_driver/CMakeLists.txt`（见 design.md §D4.2）
- [x] 创建 `plugins/iommu_driver/plugin.cpp` 骨架

### 任务 2.2：迁移 6 个 iommu framework 源文件

- [x] **Write test**: `tests/plugins/test_iommu_driver_standalone.cpp::test_iommu_init` 验证 iommu_emu_init 工作
- [x] **Verify fail**: 测试失败
- [x] **Implement**:
...
- [x] **Verify pass**: 测试通过
- [x] **Commit**: `feat(iommu-driver): migrate 6 framework files from src/kernel/iommu/`

### 任务 2.3：迁移 vfio_bridge（OQ5 决议 A）

- [x] **Write test**: `tests/plugins/test_iommu_driver_standalone.cpp::test_vfio_bridge_init`
- [x] **Verify fail**: 测试失败
- [x] **Implement**:
...
- [x] **Verify pass**: 测试通过
- [x] **Commit**: `feat(iommu-driver): migrate vfio_bridge.cpp/h`

### 任务 2.4：迁移 pcie_integration（跨 driver 协作）

- [x] **Write test**: `tests/plugins/test_iommu_driver_standalone.cpp::test_pci_iommu_integration`
- [x] **Verify fail**: 测试失败
- [x] **Implement**:
...
- [x] **Verify pass**: 测试通过
- [x] **Commit**: `feat(pci-driver): add pci_iommu_integration from src/kernel/iommu/`

### 任务 2.5：迁移 iommu_internal.h（OQ6 决议 A）

- [x] **Write test**: `tests/plugins/test_iommu_driver_standalone.cpp::test_internal_header_exists`
- [x] **Verify fail**: 测试失败
- [x] **Implement**:
...
- [x] **Verify pass**: 测试通过 + 现有 4 个 test_iommu_* 全 PASS
- [x] **Commit**: `feat(iommu-driver): migrate iommu_internal.h + fix test refs`
...
- [x] **Write test**: `tests/plugins/test_iommu_driver_standalone.cpp::test_invalidate_stays_in_q2`
- [x] **Verify fail**: 测试失败（invalidate.cpp 尚未迁移）
- [x] **Implement**:
...
- [x] **Verify pass**: 测试通过 + iommu.cpp 编译通过
- [x] **Commit**: `feat(iommu-driver): migrate invalidate.cpp (stay in Q2, defer Q2/Q3 split to Change-2)`

### 任务 2.7：移除 src/kernel/iommu/ 旧文件

- [x] **Write test**: `grep -r "src/kernel/iommu/" --include="*.cpp" --include="*.h"` 应返回 0 匹配
- [x] **Verify fail**: 测试失败
- [x] **Implement**:
  - [ ] `rm -rf src/kernel/iommu/`
  - [ ] 修改 `src/kernel/CMakeLists.txt`：移除 iommu 引用
- [x] **Verify pass**: 全仓编译通过 + 全部现有测试 PASS
- [x] **Commit**: `refactor(kernel): remove src/kernel/iommu/ after migration`
...
- [x] 创建 `sim_hardware/` 顶级目录（不在 src/ 或 include/ 下）
- [x] 创建 `sim_hardware/include/{pcie,cpptlm,chipset}/` 子目录（**不**创建 dma/，因 invalidate.cpp 暂留 Q2）
- [x] 创建 `sim_hardware/src/` 目录
- [x] 创建 `sim_hardware/topology/` 目录
- [x] 创建 `sim_hardware/CMakeLists.txt`（见 design.md §D4.3）
- [x] 创建 `sim_hardware/include/platform.h` 占位头文件

### 任务 3.2：占位头文件（Stage 5.5.1 仅骨架）

- [x] 创建 `sim_hardware/include/pcie/bypass.h` 占位（仅 enum-only，不含类方法以避免半成品 API）
- [x] 创建 `sim_hardware/include/pcie/host_bridge.h` 占位
- [x] 创建 `sim_hardware/include/cpptlm/bridge.h` 占位
- [x] **不创建** `sim_hardware/include/dma/iommu_hw.h`（invalidate.cpp 暂留 Q2，详见任务 2.6）

### 任务 3.3：顶层 CMakeLists 集成

- [x] 修改顶层 `CMakeLists.txt`：添加 `add_subdirectory(sim_hardware)`（顶级目录路径，无 `src/` 前缀）
- [x] 验证编译通过（即使占位）
...
- [x] **Write test**: `tests/test_moduleloader_toposort_standalone.cpp::test_module_struct_load_priority`
- [x] **Verify fail**: 测试失败（无 load_priority 字段）
- [x] **Implement**:
  - [ ] 修改 `include/kernel/module_loader.h`：struct module 加 `uint32_t load_priority`
- [x] **Verify pass**: 测试通过
- [x] **Commit**: `feat(moduleloader): add load_priority field to struct module (ABI change)`

### 任务 4.2：拓扑排序算法

- [x] **Write test**: 5 个测试用例（见 design.md §D5.2）
- [x] **Verify fail**: 测试失败（无拓扑排序）
- [x] **Implement**:
  - [ ] 修改 `src/kernel/module_loader.cpp`：添加 topological_sort + cycle detection
  - [ ] 修改 `load_plugins()` 使用拓扑排序
- [x] **Verify pass**: 全部 5 个测试用例通过
- [x] **Commit**: `feat(moduleloader): implement topological sort + cycle detection`
...
- [x] **Verify**: `tests/test_pcie_emu_standalone.cpp` 仍编译（路径不变）
- [x] **Verify**: `tests/test_pcie_gpu.cpp` 仍编译
- [x] **Verify**: `tests/test_iommu_emu_standalone.cpp` 仍编译
- [x] **Verify**: `tests/test_iommu_invalidate_runtime_standalone.cpp` 仍编译
- [x] **Verify**: `tests/test_iommu_notifier_standalone.cpp` 仍编译
- [x] **Verify**: `tests/test_iommu_priv_contract_regression_standalone.cpp` 仍编译（OQ6 决议 A：改 #include）
- [x] **Verify**: 全部 6 个测试 PASS

### 任务 5.2：ADR-036 升级 v0.2

- [x] 修改 `docs/00_adr/adr-036-three-way-separation.md`：3 区分 → 4 象限
- [x] 在 ADR 顶部记录 v0.2 changelog
- [x] Status: 🔄 Proposed → ✅ Accepted（由 ADR-091 v0.2 评审合并批准）

### 任务 5.3：ADR-089 升级 v0.6（location 更新）

- [x] 修改 `docs/00_adr/adr-089-v55-system-hw-simulation.md`：location `src/system_hw/` → `sim_hardware/`
- [x] Status: ✅ Accepted → ✅ Accepted v0.6

### 任务 5.4：ADR-072 升级 v0.2（L2 build 扩展）

- [x] 修改 `docs/00_adr/adr-072-portability-validation.md`：L2 build 目标集扩展
- [x] Status: ✅ Accepted → ✅ Accepted v0.2

### 任务 5.5：post-refactor-architecture.md §1.10 更新

- [x] 添加 Stage 5.5.1 完成状态标注
- [x] 列出 4 象限迁移后的目录结构（与 design.md §D1.1 对齐）
...
- [x] **测试链接方式**：**现有 6 个 test_*_standalone 通过 dlopen 加载 plugin .so 解析符号**（而非链接期 link 插件）
- [x] **新增 task**：每个 test CMakeLists.txt 加 `dlopen` helper 调 `module mod` + 解析符号
- [x] 执行：`ctest --output-on-failure`
- [x] 期望：所有现有测试 PASS（0 regression）
- [x] 基线数字：`ctest -N | wc -l`（建立 baseline 数字）

### Gate 5.5.1-B：ModuleLoader 拓扑排序工作

- [x] 执行：`./build/bin/test_moduleloader_toposort_standalone`
- [x] 期望：5 个测试场景全部 PASS

### Gate 5.5.1-C：L2 build 通过（v0.2 修订 — ORACL-Block-3 fix）

- [x] Stage 5.5.1 内**新建** `tools/l2-build/build_pci_driver.sh` + `build_iommu_driver.sh`（脚本本身在 Change-1 内实施）
- [x] 执行：`./tools/l2-build/build_pci_driver.sh`
- [x] 期望：编译通过（仅 #include sed 替换 + stub 真机编译）
- [x] 执行：`./tools/l2-build/build_iommu_driver.sh`
- [x] 期望：编译通过

### Gate 5.5.1-D：Oracle 实施后复审（v0.2 新增 — METIS-Q1.3 闭环）

- [x] Change-1 全部 Wave 完成后，**Oracle 重新审查**
- [x] Oracle 评估：(a) 实施 vs ADR-091 v0.2 + design.md 目标态的偏差
- [x] Oracle 决定：(b) 是否调整 ADR-091 / 4 象限架构文档 / 实施路线图
- [x] Oracle 决定：(c) 是否调整 Change-2（Stage 5.5.2）的内容或时间线
- [x] 输出：Oracle 复审报告（含调整建议列表 + 优先级）
- [x] **前置评审**：ADR-036/089/072 升档目标态在 Change-1 实施**前**由本次 Oracle+Metis 双审查一并审
