# add-abi-dispatch-consistency-test

**优先级**: P2 | **来源**: System C IOCTL 端到端测试完备性审计（2026-08-02）
...
- **In Scope**:
  ...
  - 测试逻辑：
    1. 构造 `GpgpuDevice`（无需 HAL/无 plugin 加载，直接静态调用 `getIoctlTablePtr()` 即可）
    2. 收集表中所有 `request` 值，排序
    3. 与 `ABI_IOCTL_REQUESTS` 排序后逐项 `REQUIRE` 等值
    4. Catch2 SECTION：缺失值报告 / 多余值报告 / 重复检测
...
- **GIVEN** 派发表某项 handler 指针为 nullptr（防御性场景）
  **WHEN** 跑同一 ctest
  **THEN** FAIL（按 Out Scope 不强制，但若有 SECTION 顺手校验，可选）

- **GIVEN** 任何场景
  **WHEN** 测试运行
  **THEN** 在 100ms 内完成（不加载 plugin/HAL/sim，纯静态枚举）
...
- [ ] `tests/test_ioctl_abi_dispatch_consistency_standalone.cpp` CTest-registered 并 PASS（前提：P0 已合，活跃派发表 38 项）
- [ ] `ABI_IOCTL_REQUESTS` 数组含 38 个值，按数值升序，每个值后有 ABI 名注释
- [ ] `REQUIRE(std::is_sorted(ABI_IOCTL_REQUESTS.begin(), ABI_IOCTL_REQUESTS.end()))` PASS
- [ ] `REQUIRE(GpgpuDevice::dispatchCount() == 38)` PASS
- [ ] 派发表 request 值集合 == `ABI_IOCTL_REQUESTS`（`REQUIRE` 逐项等值 + 总大小）
- [ ] 派发表无重复 request（`REQUIRE(std::adjacent_find(...) == end)`）
- [ ] 派发表无 nullptr handler（防御性 SECTION 可选）
- [ ] 测试运行时间 ≤ 100ms（不加载 plugin/HAL/sim）
- [ ] 模拟"新增 ABI 但漏接派发表"：临时注释掉派发表某项 → 测试 FAIL，CI 拦截
- [ ] 模拟"误删 ABI 但保留派发表"：临时在数组中删除某值 → 测试 FAIL，CI 拦截
- [ ] 既有 ctest 全 PASS，0 regression
- [ ] `plugins/gpu_driver/shared/gpu_ioctl.h` ABI 头未变
- [ ] `plugins/gpu_driver/drv/gpgpu_device.cpp` 派发表未变（除非 P0 同步合入）
- [ ] `docs/00_adr/` 无新增 ADR（不需新架构决策）
- [ ] AGENTS.md 增加 1 行说明"ABI—派发表漂移检测（test_ioctl_abi_dispatch_consistency）"
- [ ] commit message 引用 openspec 事故 + 说明 P0 协同合入
